...
class ChatListSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        user_status = find_dict_in_list(self.context.get('users_status'), 'id', user_id)

        if user_info:
            # TODO: добавил аватарку и на блоге тоже в user_list.js
            avatar_name = user_info['avatar'].split('/')[-1]
            user_info['avatar'] = (
                'http://localhost:8000/media/no_ava/no_ava.png'
                if avatar_name == 'no-image-available.jpg'
                else user_info['avatar']
            )
